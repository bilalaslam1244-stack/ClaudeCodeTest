...
async def download_and_transcribe(
    ...
    try:
        ...
        transcript = await whisper_service.transcribe(tmp_path, lang=lang)
        return transcript
    finally:
...
async def download_audio_file(
    ...
    """Download audio to a temp file and return the path. Caller must delete."""
    message = update.effective_message
    ...
    if message.voice:
        ...
        tg_file = await context.bot.get_file(message.audio.file_id)
        suffix = os.path.splitext(message.audio.file_name or ".m4a")[1] or ".m4a"
    elif message.document:
        tg_file = await context.bot.get_file(message.document.file_id)
        suffix = os.path.splitext(message.document.file_name or ".ogg")[1] or ".ogg"
    else:
